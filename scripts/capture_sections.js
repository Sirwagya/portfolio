...
function snap(name, width, height, url = 'http://127.0.0.1:5173/') {
  ...
  try {
    execSync(`"${CHROME}" --headless --disable-gpu --user-data-dir="${profileDir}" --virtual-time-budget=1500 --window-size=${width},${height} --screenshot="${outFile}" "${url}"`, { stdio: 'ignore' });
...
snap('final_desktop_playground', 1440, 1100, 'http://127.0.0.1:5173/#playground');
